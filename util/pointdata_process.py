...
print(f"[TRACE] get_pos_sample_idx loaded from {__file__}")
...
#     """
#     类别均衡正样本采样 & 不足补齐：
#       - cnts: 各 block 的点数列表，支持 Python list 或 1D Tensor
#       - num_samples: 每 block 希望采的正样本数 M
#       - labels: 伪标签 1D Tensor, 长度 = sum(cnts)
#       - num_classes: 类别总数 C
#
#     返回：长度 == M * len(cnts) 的 LongTensor，全局下标，可直接用于 embed_u[idxos][pos_idx] 等。
#     """
#     device = labels.device
#     total_pts = labels.size(0)
#
#     # 1) 将 cnts 统一成 Python list[int]
#     if isinstance(cnts, torch.Tensor):
#         cnts_list = cnts.detach().cpu().tolist()
#     else:
#         cnts_list = list(cnts)
#     B = len(cnts_list)
#     M = num_samples * B
#     per_cls = num_samples // num_classes
#
#     sample_idx = []
#     offset = 0
#
#     for b, cnt in enumerate(cnts_list):
#         # 强制检查
#         assert isinstance(cnt, int), f"cnts_list[{b}] must be int, got {type(cnt)}"
#         assert offset + cnt <= total_pts, (
#             f"Block overflow: offset({offset})+cnt({cnt}) > total_pts({total_pts})"
#         )
#         cur = labels[offset: offset + cnt]
#         for cls in range(num_classes):
#             sidx = torch.nonzero(cur == cls, as_tuple=False).view(-1)
#             n = sidx.numel()
#             if n == 0:
#                 continue
#             if n >= per_cls:
#                 pick = sidx[torch.randperm(n, device=device)[:per_cls]]
#             else:
#                 pick = sidx[torch.randint(0, n, (per_cls,), device=device)]
#             sample_idx.append(pick + offset)
#         offset += cnt
#
#     K = sum(x.numel() for x in sample_idx)
#     rem = M - K
#     if rem > 0 and total_pts > 0:
#         extra = torch.randint(0, total_pts, (rem,), device=device)
#         sample_idx.append(extra)
#
#     if not sample_idx:
#         return torch.empty((0,), dtype=torch.long, device=device)
#     result = torch.cat(sample_idx, dim=0)
#     # 最终长度校验
#     assert result.numel() == M, (
#         f"Result size {result.numel()} != expected {M}"
#     )
#     return result
#
# import random
# import torch
##修改后能够直接使用的代码，只是到4000 多步的时候会出现报错
# def get_pos_sample_idx(cnts, num_samples, labels, num_classes):
#     """
#     纯 Python + CPU 版：
#       - cnts: 各 block 的点数列表，Python list[int] 或 1D Tensor
#       - num_samples: 每 block 目标采样数 M (int)
#       - labels: 1D Tensor（长 = sum(cnts)），在任意 device 上；函数内会搬到 CPU
#       - num_classes: 类别数 C (int)
#
#     返回：长度 == M * len(cnts) 的 LongTensor，放到 labels 原来所在的 device 上。
#     """
#     # 1) 把 labels 拷到 CPU，并转成 Python 列表
#     if isinstance(labels, torch.Tensor):
#         labels_cpu = labels.detach().cpu().tolist()
#     else:
#         labels_cpu = list(labels)
#     total_pts = len(labels_cpu)
#
#     # 2) cnts -> Python list[int]
#     if isinstance(cnts, torch.Tensor):
#         cnts_list = [int(x) for x in cnts.detach().cpu().tolist()]
#     else:
#         cnts_list = [int(x) for x in cnts]
#
#     B = len(cnts_list)
#     M = num_samples * B
#     per_cls = num_samples // num_classes
#
#     result_indices = []  # 全局下标 list
#
#     offset = 0
#     for cnt in cnts_list:
#         # 提取这一 block 的标签子列表
#         block_labels = labels_cpu[offset: offset + cnt]
#         # per-class 采样
#         for cls in range(num_classes):
#             # 收集所有属于 cls 的相对索引
#             rel_idxs = [i for i, lab in enumerate(block_labels) if lab == cls]
#             n = len(rel_idxs)
#             if n == 0:
#                 continue
#             if n >= per_cls:
#                 # 不放回随机抽 per_cls 个
#                 picked = random.sample(rel_idxs, per_cls)
#             else:
#                 # 放回抽补齐
#                 picked = [ random.choice(rel_idxs) for _ in range(per_cls) ]
#             # 转成全局下标
#             result_indices += [ offset + i for i in picked ]
#         offset += cnt
#
#     # 3) 全局补齐
#     rem = M - len(result_indices)
#     if rem > 0 and total_pts > 0:
#         # 放回补齐
#         for _ in range(rem):
#             result_indices.append(random.randrange(0, total_pts))
#
#     # 4) 结果校验 & 转 Tensor
#     assert len(result_indices) == M, f"[get_pos] got {len(result_indices)} != expected {M}"
#     device = labels.device if isinstance(labels, torch.Tensor) else torch.device('cpu')
#     return torch.tensor(result_indices, dtype=torch.long, device=device)

import torch

def get_pos_sample_idx(cnts, num_samples, labels, num_classes):
    """
    原地在 CUDA 上做类别平衡采样，支持 labels 传入 list 或 Tensor。
    :param cnts:  每个 block 的点数列表，可为 Python list 或 1D Tensor
    :param num_samples: 每 block 目标采样总数 M（int）
    :param labels:  所有点的标签序列，传 list 或 Tensor 都行
    :param num_classes: 类别数 C（int）
    :return: 字符串长度为 B*M 的 LongTensor（device 同 labels）
    """
    # —— 1) 统一把 labels 转成 GPU Tensor ——
    ...
    for cnt in cnts_list:
        cnt = int(cnt)
        per_cls = num_samples // num_classes
        cur_labels = labels[offset: offset + cnt]  # 这时一定是 Tensor

        # 按类采样
        for cls in range(num_classes):
            mask = (cur_labels == cls)
            sidx = torch.nonzero(mask, as_tuple=False).view(-1)
            n = sidx.numel()
            if n == 0:
                continue
            if n >= per_cls:
                perm = torch.randperm(n, device=device)[:per_cls]
                sidx = sidx[perm]
            else:
                # 不足时放回抽
                extra = per_cls - n
                r = torch.randint(high=n, size=(extra,), device=device)
                sidx = torch.cat([sidx, sidx[r]], dim=0)
            sample_idx.append(sidx + offset)

        # 剩余不满 per_cls*classes 的，随便补齐
        taken = sum(t.numel() for t in sample_idx[-num_classes:])
        rem = num_samples - taken
        if rem > 0:
            r = torch.randperm(cnt, device=device)[:rem]
            sample_idx.append(r + offset)
