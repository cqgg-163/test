...
# 统计和记录某个数值（如loss、准确率）在训练/验证过程中的当前值、平均值、总和和计数。
...
# 算每类的交集（预测正确的数量）、并集和target总数，用于计算IoU和准确率。
...
# 以下一部分为初始化相关，辅助做权重初始化，防止深度网络训练时梯度消失/爆炸
...
def get_semantic_names(dataset):
    ...
    # elif dataset == 's3dis':
    #     semantic_label_idx = [1, 2, 3, 4, 5, 6]
    #     semantic_names = np.array([
