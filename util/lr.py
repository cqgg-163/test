from torch.optim.lr_scheduler import LambdaLR, StepLR, OneCycleLR, CosineAnnealingLR, SequentialLR
...
def initialize_scheduler(optimizer, config, last_step=-1, logger=None):
    if config.scheduler == 'StepLR':
        ...
        return PolyLR(optimizer, max_iter=config.iters, power=config.poly_power, last_step=last_step)
    elif config.scheduler == 'SquaredLR':
        return SquaredLR(optimizer, max_iter=config.iters, last_step=last_step)
        ...
        if config.get('warmup', False):
            # 定义一个线性的 warmup 函数
            warmup_fn = lambda it: (it + 1) / config.warmup_iter
            warmup_scheduler = LambdaLR(optimizer, lr_lambda=warmup_fn)

            # 使用 SequentialLR 将 warmup 和主 scheduler 串联
            return SequentialLR(
                optimizer,
        ...
        return OneCycleLR(optimizer, max_lr=config.oc_max_lr, total_steps=config.iters, pct_start=config.oc_pct_start,
