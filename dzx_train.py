...
def init_dist_pytorch(backend='nccl'):
    ...
    cfg.local1_rank = int(os.environ['LOCAL_RANK'])
...
#用于读取批次数据，并涉及分布式计算，每个epoch都要用sampler.set_epoch(epoch)保证每个GPU读到的数据不同。
...
#设置模型为评估模型（不会更新参数）
#用torch.no_grad()防止梯度计算提高效率。
#分布式时同步多个GPU的指标（all_reduce），保证评估结果准确。
#
