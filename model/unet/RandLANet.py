...
class Network(nn.Module):
    ...
    def __init__(self, config):
        ...
        for i in range(self.config.num_layers):
            ...
            print(d_in, d_out,"112-------------")
            print(Dilated_res_block(d_in, d_out),"di")
        ...
        for j in range(self.config.num_layers):
            ...
            if j < config.num_layers - 1:
                print(d_in, d_out,"111-------------")
                d_in = d_out + 2 * self.config.d_out[-j-2]          # -2是因为最后一层的维度不需要拼接 乘二还是因为实际的输出维度是2倍的dout # din=1024+512 维度增加是因为进行了拼接
                d_out = 2 * self.config.d_out[-j-2]                 # 通过解码器里面的MLP调整回对应层的维度
            else:
                print(d_in, d_out,"222-------------")
    ...
    def forward(self, end_points):
        ...
        # print('fc0.in_features:', self.fc0.in_features)
        # print('fc0.weight.device:', self.fc0.weight.device)
        #
        # features = self.fc0(features)
        # # print(features.shape, features.dtype, features.device)
        # # print(self.fc0.weight.shape, self.fc0.weight.dtype, self.fc0.weight.device)
        # # 下面三行是后面改的
        ...
        # Linear expects 2D, so flatten batch and points
        features = features.view(B * N, C)  # [B*N, 6]
        features = self.fc0(features)  # [B*N, 8]
        features = self.fc0_acti(features)  # [B*N, 8]

        # BatchNorm1d expects [N, C] or [B, C, L] (用 [B*N, 8] 没问题)
        features = self.fc0_bath(features)  # [B*N, 8]

        # 还原回 [B, N, 8]
        features = features.view(B, N, 8)
        features = features.transpose(1, 2)  # [B, 8, N]
        features = features.unsqueeze(dim=3)  # [B, 8, N, 1]
